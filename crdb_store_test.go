...
func testStore(t *testing.T) (*CRDBStore, *sql.DB, context.Context) {
	t.Helper()
	dsn := os.Getenv("LEASES_TEST_DSN")
	if dsn == "" {
		t.Skip("LEASES_TEST_DSN not set")
	}
...
func TestDeleteActiveLeaseBlocked(t *testing.T) {
	store, db, ctx := testStore(t)
...
func TestReleaseWithWrongToken(t *testing.T) {
	store, db, ctx := testStore(t)
...
func TestHeartbeatMany(t *testing.T) {
	store, db, ctx := testStore(t)
...
func TestHeartbeatManyEmpty(t *testing.T) {
	store, db, ctx := testStore(t)
